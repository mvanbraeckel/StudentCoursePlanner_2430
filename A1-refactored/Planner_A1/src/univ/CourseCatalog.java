...
public class CourseCatalog implements Serializable
{
        ...
        try {
            ...
            while(!eof) {
                ...
                if(line == null) {
                    ...
                    // check that CSV line had proper number of values
                    if(lineSplit.length != 5) {
                        System.out.println("Error: initializeCatalog(" + filename + "): CSV file line '" + line + "' is invalid");
                        validInput = false;
                    ...
                    if(codeSplit.length != 2 || codeSplit[0].length() < 3 || codeSplit[0].length() > 4 || 
                        ...
                        System.out.println("Error: initializeCatalog(" + filename + "): course code '" + lineSplit[0] + "' is invalid");
                        validInput = false;
                        continue; //do not add to catalog because invalid course code format
                    }
                    ...
                    if(!isDouble(lineSplit[1])) {
                        System.out.println("Error: initializeCatalog(" + filename + "): course credit '" + lineSplit[1] + "' is invalid");
                        validInput = false;
                    ...
                    if(lineSplit[2].equals("")) {
                        System.out.println("Error: initializeCatalog(" + filename + "): course title must exist");
                        validInput = false;
                        continue; //do not add to catalog because invalid credit
                    }
                    
                    // check semester offered
                    if(!lineSplit[3].equals("F") && !lineSplit[3].equals("W") && !lineSplit[3].equals("B") && !lineSplit[3].equals("S")) {
                        System.out.println("Error: initializeCatalog(" + filename + "): semester offered '" + lineSplit[3] + "' is invalid");
                        validInput = false;
                        continue; //do not add to catalog because invalid credit
                    }
                    ...
                    if(lineSplit.length == 5) {
                        ...
                        for(String code : prereqSplit) {
                            ...
                            if(prereqCodeSplit.length != 2 || prereqCodeSplit[0].length() < 3 || prereqCodeSplit[0].length() > 4 || 
                                ...
                                System.out.println("Error: initializeCatalog(" + filename + "): prerequisite course code '" + code + "' is invalid");
     ...
     */
    public void readSavedCatalog(String filename) {
        boolean eof;
